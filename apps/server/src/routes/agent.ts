...
const DRIVER_PROMPT = [
  ...
  '19.（第 16 步）**禁止**向用户抛「原任务是 X、最新指令是 Y，请问你要做 A 还是 B」这种二选一：用户改口后旧目标即作废，按最新指令直接出下一步动作。',
  '20.（第 16 步）ask_user 只在**真的被卡住**时用（要登录 / 遇到验证码 / 页面上没有可用入口且无法自行判断 / 缺必要资料），且问题要问「这一步该怎么走」，不要问「选哪个目标」。',
  '21.（第 16 步）目标说的是「在**当前这张**页面上做一件事」（在这个页面搜一下 X / 读一下当前页 / 往下滚 / 点某处 / 翻到下一页）时：**不要 open_url 换站点**，直接在 snapshot 里找对应元素操作（先找搜索框 → type 关键词 → 提交）。只有目标明确要换站点或当前页明显不是目标站点时，才允许 open_url。',
].join('\n');
