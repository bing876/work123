...
- **目录改名/移动报 `EPERM`（`Permission denied`）时，用"逐层试改名"定位被占用的那一层，
  别去猜进程**：外层目录失败、内层目录与单个文件都成功 → 就是**外层目录本身**被某进程持有句柄
  （内容不受影响）。绕法 = **先搬内容、再删空壳**（空目录的 `rmdir` 通常立刻成功）。
  实测样例见 `2026-09-18.md` §四。
- **`git mv` 的目标若已存在同名目录，语义是"移进去"**（POSIX `mv` 语义，会变成
  `目标/源名`）—— 先把目标路径确认不存在，或直接把目标拼到最终文件名。
...
## 六、工作区记忆目录（目录名已直接对齐平台默认值，无需任何额外处理）

**读写记忆落在 `.workbuddy/memory/`** —— 这既是客户端自己会写的目录，也是仓库**已跟踪**的目录。两边同名，**天然一致**。

- **目录名由来**：客户端按 `product.json#dataFolderName`（当前 = `.workbuddy`）拼出
  `${cwd}/${dataFolderName}/memory/` 当工作区记忆目录（`app.asar` 里就是
  `folder = process.env.WORKBUDDY_DATA_FOLDER_NAME?.trim() || ".workbuddy"`）。
  本仓库**已把跟踪的目录改成同一个名字**，所以平台写什么、仓库就跟踪什么。
- **路径名变更说明**：**2026-09-18 之前**记忆记录路径是 `.workbuddy-ai/memory/`（海外版的目录名，
  与国内客户端的 `.workbuddy` 不同名，曾导致同一天出现两份 `2026-09-17.md`）；
  **2026-09-18 起统一为 `.workbuddy/memory/`**。历史提交里的旧路径名**不改写、不追改历史**，
  只在此说明；要看旧路径下的内容，直接查历史提交或 `2026-09-17.md` 的归并记录。
- **不需要任何联接 / 脚本 / 平台特有技巧**：曾短暂用过 Windows **目录联接（junction）**把两个名字
  接起来，缺点是 Windows 专属（Mac 要换 symlink），且**每台新机器、每次克隆都要手动重建**，
  一旦遗忘分裂就会重演 —— 已废弃并改名为直接对齐（一次性 `git mv`）。**不要再引入任何链接方案。**
- **唯一要防的**：别把记忆目录名字改回 `-ai` 后缀。若哪天客户端把 `dataFolderName` 换成别的值，
  分裂会以同样方式复发 —— 判别手法：会话里注入的记忆路径若是 `<repo>/<某个点目录>/memory/`
  且不等于 `.workbuddy`，就是又变了（查 `app.asar` 里 `WORKBUDDY_DATA_FOLDER_NAME` /
  `dataFolderName` 的当前值），届时再 `git mv` 对齐即可。
- 若客户端日后往 `<repo>/.workbuddy/` 里写**别的**运行时文件：按需在 `.gitignore` 里忽略**具体的
  子路径**，不要为了省事把整个 `.workbuddy/` 忽略了（那会让记忆记录重新变成未跟踪）。
